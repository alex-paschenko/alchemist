...
function passOutSlices<T>(
  ...
  const result: (T[] | null)[] = new Array(passOutParamRules.length).fill(null);
  let remainder: T[] = [...inputArray];
      ...
      if (typeof rule === 'number') {
        if (rule === 0) {
          ...
          if (remainder.length < rule) {
            throw new AlchemistError('E100', index, rule);
          }
  ...
  if (remainder.length > 0 && spreadIndex === -1) {
    throw new AlchemistError('E101');
  }

  // Ensure the result length matches numOfClasses
  if (result.filter(segment => segment !== null).length !== numOfClasses) {
    throw new AlchemistError('E102', numOfClasses);
  }

  return result.map(segment => segment || []) as T[][];
}


function validatePassOutParamRules(passOutParamRules: PassOutParamRule[]): void {
  /**
  ...
  if (!Array.isArray(passOutParamRules)) {
    throw new AlchemistError('E010');
  }

  if (passOutParamRules.filter(rule => rule === '...').length > 1) {
    throw new AlchemistError('E011');
  }

  passOutParamRules.forEach((rule, index) => {
    if (rule !== '...' && (typeof rule !== 'number' || !Number.isInteger(rule))) {
      throw new AlchemistError('E012', index, rule);
    }
  });
}
